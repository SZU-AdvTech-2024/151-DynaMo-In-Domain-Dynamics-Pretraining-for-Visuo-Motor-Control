...
def mse(pred, y, vars, pde, lat=None, mask=None, inp_vars=None):
    """Mean squared error
...
def lat_weighted_mse(pred, y, vars, pde, lat, mask=None, inp_vars=None):
...
def lat_weighted_mse_pde_loss(pred, y, vars, pde, weight, pde_weight, lat, mask=None, inp_vars=None):
    """Latitude weighted mean squared error
    ...
    #     gt_y  = torch.gradient(y, dim=2)[0]  # (B, C, H, W) --> gradient along H
    #     gt_x = torch.gradient(y, dim=3)[0]  # (B, C, H, W) --> gradient along W
    #     adv1 = v_x * u_x + v_y * u_y + y * (torch.gradient(v_y, dim=2)[0] + torch.gradient(v_x, dim=3)[0])
    #     adv2 = v_x * gt_x + v_y * gt_y + pred * (torch.gradient(v_y, dim=2)[0] + torch.gradient(v_x, dim=3)[0])
    #     return ((delta_u - adv1)) ** 2 +  ((delta_u - adv2)) ** 2 
...
def lat_weighted_mse_pde_loss__1(preds, embed_U, y, vars, pde, weight, pde_weight, lat, mask=None, inp_vars=None):
    """Latitude weighted mean squared error
    ...
    def pde_loss(pred, pde):
        ...
        u_y = torch.gradient(pred, dim=2)[0]  # (B, C, H, W) --> gradient along H
        u_x = torch.gradient(pred, dim=3)[0]  # (B, C, H, W) --> gradient along W
        adv = v_x * u_x + v_y * u_y + pred * (torch.gradient(v_y, dim=2)[0] + torch.gradient(v_x, dim=3)[0])
        
        return (delta_u - adv) ** 2

    loss_pde = pde_loss(embed_U, pde)
    ...
    with torch.no_grad():
        for i, var in enumerate(vars):
            if mask is not None:
                ...
                loss_dict[var] = (err[:, i] * w_lat).mean() 
    ...
    if mask is not None:
        ...
        loss_dict["loss"] =  (err * w_lat.unsqueeze(1)).mean(dim=1).mean() + weight * (error * w_lat.unsqueeze(1)).mean(dim=1).mean() + pde_weight * (loss_pde * w_lat.unsqueeze(1)).mean(dim=1).mean() 
...
def lat_weighted_mse_val(pred, y, transform, vars, pde, lat, clim, log_postfix, inp_vars=None):
    """Latitude weighted mean squared error
    Args:
        y: [B, V, H, W]
        pred: [B, V, H, W]
...
def lat_weighted_rmse(pred, y, transform, vars, pde, lat, clim, log_postfix, inp_vars=None):
...
def lat_weighted_acc(pred, y, transform, vars,pde,  lat, clim, log_postfix, inp_vars=None):
    """
    y: [B, V, H, W]
    pred: [B V, H, W]
    vars: list of variable names
...
def lat_weighted_nrmses(pred, y, transform, vars, pde, lat, clim, log_postfix, inp_vars=None):
    """
    y: [B, V, H, W]
    pred: [B V, H, W]
    vars: list of variable names
...
def lat_weighted_nrmseg(pred, y, transform, vars, pde, lat, clim, log_postfix, inp_vars=None):
    """
    y: [B, V, H, W]
    pred: [B V, H, W]
    vars: list of variable names
...
def lat_weighted_nrmse(pred, y, transform, vars, pde, lat, clim, log_postfix, inp_vars=None):
    """
    y: [B, V, H, W]
    pred: [B V, H, W]
    vars: list of variable names
...
def pearson(pred, y, transform, vars,pde,  lat, log_steps, log_days, clim):
    """
    y: [N, T, 3, H, W]
    pred: [N, T, 3, H, W]
    vars: list of variable names
...
def lat_weighted_mean_bias(pred, y, transform, vars, pde, lat, log_steps, log_days, clim, inp_vars=None):
    """
    y: [N, T, 3, H, W]
    pred: [N, T, 3, H, W]
    vars: list of variable names
