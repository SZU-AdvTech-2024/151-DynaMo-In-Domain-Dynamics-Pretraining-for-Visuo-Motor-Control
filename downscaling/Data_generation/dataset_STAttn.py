...
class Forecast(IterableDataset):
    ...
    def __iter__(self):
        for data, variables, out_variables in self.dataset:
            ...
            inputs = x[7000: -self.max_predict_range]
...
class myForecast(IterableDataset):
    ...
    def __iter__(self):
        for data, variables, out_variables in self.dataset:
            ...
            inputs = x[6000: 6801]
            inputs = inputs[0: -self.max_predict_range]
...
class myIndividualForecastDataIter(IterableDataset):
        ...
        for (inp, out, lead_times, variables, out_variables) in self.dataset:
            ...
            for i in range(skip*frame, inp.shape[0]):
                if self.region_info is not None:
                    ...
                    yield series_inp, self.output_transforms(out[i]), lead_times[i], variables, out_variables, self.region_info
                else:
                    series_inp = self.transforms(inp[i]).unsqueeze(0)
                    for j in range(1, frame):
                        series_inp = torch.cat((series_inp, self.transforms(inp[i - skip*j]).unsqueeze(0)), dim=0)
                    yield series_inp, self.output_transforms(out[i]), lead_times[i], variables, out_variables
