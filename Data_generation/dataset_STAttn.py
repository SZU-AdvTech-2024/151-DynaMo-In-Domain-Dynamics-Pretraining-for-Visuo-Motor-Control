...
class Forecast(IterableDataset):
    ...
    def __iter__(self):
        for data, variables, out_variables in self.dataset:
            ...
            inputs = x[0: -self.max_predict_range]  # N, C, H, W
            # print(inputs.shape)
            # return 
...
class myForecast(IterableDataset):
    def __init__(
        self, dataset: NpyReader, max_predict_range: int = 6, random_lead_time: bool = False, hrs_each_step: int = 1, start=6000
    ) -> None:
        super().__init__()
        self.dataset = dataset
        self.max_predict_range = max_predict_range
    ...
    def __iter__(self):
        for data, variables, out_variables in self.dataset:
            ...
            yield inp, variables, out_variables
...
class IndividualForecastDataIter(IterableDataset):
    ...
    def __iter__(self):
        for (inp, variables, out_variables) in self.dataset:
            ...
            frame = 6
            skip = 1
            for i in range(0, inp.shape[0] - frame + 1):
                series_inp = self.transforms(inp[i]).unsqueeze(0)

                for j in range(1, frame):
                    series_inp = torch.cat((series_inp, self.transforms(inp[i + skip*j]).unsqueeze(0)), dim=0)
                series_inp = series_inp.unsqueeze(1) #N,T,V,C,H,W
                yield series_inp, variables, out_variables
